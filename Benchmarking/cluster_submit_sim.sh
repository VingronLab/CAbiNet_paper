...
# Set dataset to either "pbmc3k" or "zeisel"
dataset="pbmc3k"
...
## Parameters shared by algorithms
ntop=(2000 4000 6000)
nclust=6
truth='Group'
...
# Loop over the files for benchmarking
...
for f in ${files[@]}; do
            ...
            for gs in ${graph_select[@]}; do
                ...
                for snn in ${SNN_mode[@]}; do
                    ...
                    for gc in ${calc_gckNN[@]}; do
                        ...
                        nm="${algorithm}_${dataset}_${filename}_ntop-${nt}_dims-${d}_NNs-${n}_gs-${gs}_SNN-${snn}_gcKNN-${gc}_overlap-${ov}"
    ...
    for r in ${r_param[@]}; do
        for q in ${q_param[@]}; do
            for c in ${c_param[@]}; do
                ...
                nm="${algorithm}_${dataset}_${filename}_ntop-${nt}_rparam-${r}_qparam-${q}_cparam-${c}"
    ...
    for pv in ${pcerv[@]}; do
        ...
        nm="${algorithm}_${dataset}_${filename}_ntop-${nt}_pcerv-${pv}_pceru-${pu}_ssthrmin-${ss}_ssthradd-${sa}"
    ...
    for rr in ${rrelease[@]}; do
        ...
        nm="${algorithm}_${dataset}_${filename}_ntop-${nt}_rrelease-${rr}_crelease-${cr}"
    ...
    for tp in ${t_param[@]}; do
        ...
        nm="${algorithm}_${dataset}_${filename}_ntop-${nt}_tparam-${tp}_qdiscr-${qd}"
    ...
    for mr in ${minr[@]}; do
        ...
        nm="${algorithm}_${dataset}_${filename}_ntop-${nt}_minr-${mr}_minc-${mc}"
    ...
    for de in ${delta[@]}; do
        ...
        nm="${algorithm}_${dataset}_${filename}_ntop-${nt}_delta-${de}_alpha-${al}"
    ...
    for ns in ${ns_param[@]}; do
        ...
        nm="${algorithm}_${dataset}_${filename}_ntop-${nt}_nsparam-${ns}_ndparam-${nd}_sdparam-${sd}_alphaX-${ax}"
    ...
    for nE in ${numEig[@]}; do
        ...
        nm="${algorithm}_${dataset}_${filename}_ntop-${nt}_numEig-${nE}_minrBCS-${mrB}_mincBCS-${mcB}_withinVar-${wV}"
    ...
    for qCo in ${qcons[@]}; do
        ...
        nm="${algorithm}_${dataset}_${filename}_ntop-${nt}_Qconsistency-${qCo}_Qoverlap-${qOv}_Qcmin-${qCm}"
